...
# %pip uninstall -y xarray albumentations bigframes imbalanced-learn mlxtend jax albucore chex pymc scikit-image jaxlib blosc2 plotly nbformat
...
plt.title('Árbol de Decisión para Predicción de Casos COVID-19', fontsize=14, pad=20)
...
def crear_dashboard_multinivel():
    fig = make_subplots(
        ...
        subplot_titles=('Casos Globales por Continente', 'Distribución de Casos',
                       'Top 10 Países más Afectados', 'Métricas Adicionales'),
    ...
    # Vista Top 10 Países
    top_10_paises = df.nlargest(10, 'cases')
    fig.add_trace(
        go.Bar(
            name='Top 10 Países',
            x=top_10_paises['country'],
            y=top_10_paises['cases'],
            marker_color=colors,
            text=top_10_paises['cases'].apply(lambda x: f'{x/1e6:.1f}M'),
            textposition='outside',
            customdata=np.stack((
                top_10_paises['deaths'],
                top_10_paises['recovered'],
                top_10_paises['active'],
                top_10_paises['critical']
            ), axis=-1),
            hovertemplate=(
                '<b>%{x}</b><br>' +
                'Casos: %{y:,.0f}<br>' +
    ...
    num_continentes = len(df['continent'].unique())
            ...
            'buttons': [
                {
                    'method': 'update',
                    'label': 'Vista Global',
                    'args': [{'visible': [True] + [False]*num_continentes + [False, True]}]
                },
                {
                    'method': 'update',
                    'label': 'Vista Continental',
                    'args': [{'visible': [False] + [True]*num_continentes + [False, True]}]
                },
                {
                    'method': 'update',
                    'label': 'Top 10 Países',
                    'args': [{'visible': [False] + [False]*num_continentes + [True, True]}]
                }
            ],
            'direction': 'down',
            'showactive': True,
...
def crear_dashboard_multinivel():
    ...
    fig = make_subplots(
        ...
        specs=[[{"secondary_y": False}, {"type": "domain"}],
               [{"colspan": 2}, None]],
        subplot_titles=("Comparación: Casos Recuperados vs Fallecimientos de paises top 10", "Distribución",
    ...
    datos_pais = df.sort_values('cases', ascending=False).head(10)

    # Bar chart comparing recovered vs deaths
    fig.add_trace(
        go.Bar(
            name='Casos Recuperados',
            x=datos_pais['country'],
            y=datos_pais['recovered'],
            text=datos_pais['recovered'].apply(lambda x: f'{x:,.0f}'),
            textposition='auto',
            marker_color='rgb(75,181,67)',
            hovertemplate="<b>%{x}</b><br>" +
                         "Recuperados: %{y:,.0f}<br>" +
                         "<extra></extra>",
            visible=True
    ...
    fig.add_trace(
        go.Bar(
            ...
            x=datos_pais['country'],
            y=datos_pais['deaths'],
            text=datos_pais['deaths'].apply(lambda x: f'{x:,.0f}'),
            textposition='auto',
            marker_color='rgb(255,69,0)',
            hovertemplate="<b>%{x}</b><br>" +
                         "Fallecimientos: %{y:,.0f}<br>" +
                         "<extra></extra>",
            visible=True
        ),
        row=1, col=1
    ...
    fig.add_trace(
        go.Pie(
            labels=datos_pais['country'],
            values=datos_pais['cases'],
            hole=0.4,
    ...
    fig.update_layout(
        title={
            'text': 'Dashboard Recuperación vs Mortalidad paises top 10',
            'y':1.0,
            'x':0.5,
            'xanchor': 'center',
            'yanchor': 'top',
    ...
    fig.update_xaxes(title_text="Casos Recuperados", row=2, col=1)
    fig.update_yaxes(title_text="Fallecimientos", row=2, col=1)
...
"""# Dashboard COVID-19: Recuperación vs Mortalidad

## Descripción General
Dashboard interactivo que visualiza la relación entre casos recuperados y fallecimientos por COVID-19, con múltiples niveles de análisis.

## Visualizaciones Principales

### 1. Comparación de Recuperación y Mortalidad
- Gráfico de barras agrupadas
- Top 10 países con más casos
- Barras verdes: Casos recuperados
- Barras rojas: Fallecimientos
- Datos numéricos sobre cada barra

### 2. Distribución Global
- Gráfico circular (donut)
- Distribución porcentual por país
- Leyenda interactiva
- Colores distintivos por país

### 3. Análisis Comparativo
- Gráfico de dispersión
- Eje X: Casos recuperados
- Eje Y: Fallecimientos
- Escala de color: Casos por millón
- Etiquetas de países al pasar el cursor

## Uso del Dashboard

### Interacción
1. Selección de datos por país
2. Visualización de detalles al pasar el cursor
3. Filtrado por categorías
4. Zoom en áreas específicas

### Interpretación
- Comparación directa recuperados vs fallecimientos
- Tendencias por país
- Correlaciones entre variables
- Distribución porcentual de casos

## Datos Mostrados
- Casos recuperados totales
...
def crear_dashboard_multinivel():
    ...
    fig = make_subplots(
        ...
        subplot_titles=("Evolución COVID-19 por Continente", "Distribución Global",
                       "Análisis Comparativo de Países",
                       "Cobertura de Pruebas", "Tasa de Recuperación"),
    ...
    datos_continente = df.groupby('continent').agg({
        ...
        'active': 'sum',
        'population': 'sum',
        'testsPerOneMillion': 'mean'
    }).reset_index()

    datos_continente['mortality_rate'] = (datos_continente['deaths'] / datos_continente['cases'] * 100)
    datos_continente['recovery_rate'] = (datos_continente['recovered'] / datos_continente['cases'] * 100)
    ...
    fig.add_trace(
        go.Bar(
            ...
            customdata=np.stack((
                ...
                datos_continente['recovery_rate']
            ), axis=-1),
            hovertemplate=(
                "<b>%{x}</b><br>" +
                "Casos: %{y:,.0f}<br>" +
                ...
                "Recuperados: %{customdata[1]:,.0f}<br>" +
                "Activos: %{customdata[2]:,.0f}<br>" +
                "Tasa de Recuperación: %{customdata[3]:.1f}%<br>" +
                "<extra></extra>"
            ),
            visible=True
        ),
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(
                size=10,
                color=df['recovered'].div(df['cases']).multiply(100),
                colorscale='Viridis',
                showscale=True,
                colorbar=dict(title='Tasa de Recuperación (%)')
            ),
            text=df['country'],
            customdata=np.stack((
                df['deaths'],
                ...
                df['active'],
                df['critical'],
                df['recovered'].div(df['cases']).multiply(100)
            ), axis=-1),
            hovertemplate=(
                "<b>%{text}</b><br>" +
                "Tests/Millón: %{x:,.0f}<br>" +
                ...
                "Tasa de Recuperación: %{customdata[4]:.1f}%<br>" +
                "<extra></extra>"
            ),
            visible=True
        ),
    ...
    fig.add_trace(
        go.Indicator(
            mode="number+delta",
            value=df['testsPerOneMillion'].mean(),
            title={'text': "Promedio Tests por Millón"},
            delta={'reference': df['testsPerOneMillion'].median(),
                   'relative': True,
                   'valueformat': '.1%'},
    ...
    fig.add_trace(
        go.Indicator(
            mode="number+delta",
            value=(df['recovered'].sum() / df['cases'].sum() * 100),
            title={'text': "Tasa de Recuperación Global"},
            number={'suffix': "%",
                    'font': {'size': 50, 'color': '#2ca02c'},
                    'valueformat': ".2f"},
            delta={'reference': 95,
                   'relative': False},
            visible=True
    ...
    # Add continent selector menu
    buttons = [dict(
        args=[{
            "x": [datos_continente['continent'], datos_continente['continent']],
            "y": [datos_continente['cases'], datos_continente['mortality_rate']],
    ...
    for continent in datos_continente['continent'].unique():
        continent_data = datos_continente[datos_continente['continent'] == continent]
        buttons.append(dict(
            args=[{
                "x": [[continent], [continent]],
                "y": [[continent_data['cases'].values[0]], [continent_data['mortality_rate'].values[0]]],
                "visible": [True] * len(fig.data)
            }],
            label=continent,
            method="update"
    ...
    fig.update_layout(
        ...
        title={
            'text': 'Dashboard : Análisis Global y Regional',
            'y':1.0,
            'x':0.5,
            'xanchor': 'center',
            'yanchor': 'top',
...
## Descripción General
Dashboard interactivo que visualiza datos globales de COVID-19 con múltiples niveles de análisis y capacidades de filtrado.

## Características
- **Análisis Continental**: Gráfico de barras mostrando casos por continente con superposición de tasa de mortalidad
- **Distribución Global**: Gráfico de dona mostrando la distribución de casos entre continentes
- **Comparación entre Países**: Gráfico de dispersión comparando tasas de pruebas y casos por millón
- **Métricas Clave**: Indicadores dinámicos de cobertura de pruebas y tasas de recuperación
- **Selector Interactivo**: Filtro por continente para análisis regional detallado

## Componentes

### 1. Panel de Visualización Principal
- Gráfico de Barras + Línea
  - Casos por Continente (barras)
  - Tasa de Mortalidad (línea)
  - Detalles interactivos al pasar el cursor

### 2. Panel de Distribución
- Gráfico de Dona
  - Distribución porcentual de casos
  - Código de colores por continente

### 3. Análisis Comparativo
- Gráfico de Dispersión
  - Eje X: Pruebas por Millón
  - Eje Y: Casos por Millón
  - Escala de color: Tasa de Recuperación
  - Detalles específicos por país al pasar el cursor

### 4. Indicadores Métricos
- Cobertura de Pruebas
  - Promedio de pruebas por millón
  - Comparación con la mediana
- Tasa de Recuperación
  - Porcentaje global de recuperación
  - Comparación con objetivo

## Uso
1. Utilizar el selector de continente para filtrar datos
2. Pasar el cursor sobre los elementos para información detallada
3. Hacer clic en elementos de la leyenda para alternar visibilidad
4. Comparar métricas entre regiones
"""
...
def crear_dashboard_avanzado():
    ...
    fig.add_trace(line_trace, secondary_y=True, row=1, col=1)
    ...
    # Update menus configuration with fixed "Todos los Continentes" button
    buttons = [dict(
        args=[{
            "x": [datos_globales['continent'], datos_globales['continent']],
            "y": [datos_globales['cases'], datos_globales['mortality_rate']],
            "visible": [True] * len(fig.data)
        }],
        label="Todos los Continentes",
        method="update"
    ...
    for continent in datos_globales['continent'].unique():
        continent_data = datos_globales[datos_globales['continent'] == continent]
        buttons.append(dict(
            args=[{
                "x": [[continent], [continent]],
                "y": [[continent_data['cases'].values[0]], [continent_data['mortality_rate'].values[0]]],
    ...
    fig.update_layout(
        ...
        title={
            'text': 'Dashboard: Análisis Global Avanzado',
            'y': 0.98,
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top',
